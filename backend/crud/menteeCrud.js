...
const MenteeCrud = {
  ...
  async create(data) {
    ...
    const [result] = await pool.query(
      `INSERT INTO mentees (name, contact_info, interests, needs_goals, preferred_communication, location)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [name, JSON.stringify(contact_info), JSON.stringify(interests), needs_goals, preferred_communication, location]
    );
